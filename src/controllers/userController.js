...
const registerUser = async function (req, res) {
  try {
    ...
    if (!validator.isValidRequestBody(body)) {
      res
        .status(400)
        .send({ status: false, message: "value in request body is required" });
      return;
    }
    ...
    if (!validator.isValidTitle(title.trim())) {
      res.status(400).send({
        status: false,
        message: "title is not valid provide among mr,miss,mrs",
      });
      return;
    }
    ...
    if (!validator.isValidPhone(phone)) {
      return res
        .status(400)
        .send({ status: false, msg: "The phone no. is not valid" });
    }
    ...
    if (isEmailAlreadyUsed) {
      res.status(400).send({
        status: false,
        message: `${email.trim()} email is already registered`,
      });
      return;
    }
    ...
    if (!validator.isValid(email)) {
      res.status(400).send({
        status: false,
        message: "Invalid request parameters. Please provide valid email",
      });
      return;
    }
...
const login = async function (req, res) {
  try {
    ...
    if (email && password) {
      ...
      res.status(400).send({
        status: false,
        msg: "request body must contain  email as well as password",
      });
    }
  } catch (err) {
    res.status(500).send({ status: false, message: err.message });
